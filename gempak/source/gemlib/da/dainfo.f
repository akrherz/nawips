...
C*
	...
	CHARACTER	dalbl*28, label*28, key*8, key2*8, tstr*12,
     +			hdrtyp*8, cycle*(MXFLSZ), forecast*(MXFLSZ), empty*8
...
C
	hdrtyp = 'COL'
	CALL DA_GETTIME ( filnam, cycle, forecast, ier )
	CALL ST_NULL ( hdrtyp, hdrtyp, lenh, ier )
      CALL ST_NULL ( cycle, cycle, lenh, ier )
      CALL ST_NULL ( forecast, forecast, lenh, ier )
	CALL DA_GETHEADER ( iflno, hdrtyp, cycle, forecast, kcol(iflno), colhdr, ier )
	icl = 1
	DO  nn = 1, kcol(iflno)
	    ihd = ihd + 1
...
C*
	...
	SUBROUTINE DA_GETTIME ( filnam, cycle, forecast, iret )
C************************************************************************
C* DA_GETTIME                                                           *
C*                                                                      *
...
C************************************************************************
      ...
      CHARACTER*(*)  filnam, cycle, forecast
...
C         CALL TG_CFTM  ( intdtf(3), ftype, forecast, iret )
         ihm = MOD ( intdtf(3), 100000 )
         ih = ihm / 100
         im = MOD ( ihm, 100 )
         ifcst = ( ih * 60 + im ) * 60
         CALL ST_INCH ( ifcst, forecast, ier )
      ENDIF
