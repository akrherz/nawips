...
C* DECOMIF								*
C*									*
C* This program makes program documentation for the decoders.		*
C*									*
...
C*									*
C* For example:  decomif 6+dcuari+h+../tpl+/usr/doc/mif			*
C*									*
C* Input file names:  $GEMHLP/dcuair.hlp				*
C*                    tplPath/tpl_chapX.mif				*
C*									*
C* Output file name:  /usr/doc/mif/dcuair.prgmif			*
C**									*
C* Log:									*
C* M. Linda/GSC		10/96	Based on parmmif.f			*
...
C
	...
	outfil = cwords (5) (1:lensw) // '/' // parm (1:isparm)
     +		 // '.decmif'
	txtfil = 'GEMHLP:'// parm (1:isparm) // '.hlp'
C
...
C
	    ...
	    CALL WTSTUP ( lunout, luntpl, ier )
            ...
            CALL WTLINE ( lunout, 'GEMPAK Real-time Decoders', ier)
	ELSE
	    CALL SETOPN ( 1, ier )
	END IF
C
...
C
	...
	IF ( num .eq. 1 )  THEN
	    out = '\t' // cwords(1)
	ELSE
	    CALL ST_LSTR ( cwords(1), ilstr, ier )
	    out = '\t' // cwords(1)(1:ilstr) // ' ' // cwords(2)
	END IF
...
C*	'ienm .eq. 1' means it is a list
...
C*	Change to list2 format
		...
		ELSE
			...
			prvlst = 'Body'
		    END IF
		    IF ( record(1:1) .eq. CHTAB .and. 
     +			 nb .ge. 1 ) THEN
  		        CALL WTPARA ( lunout, 'tabit', ier )
		        prvlst = 'tabit'
			CALL WTLINE ( lunout, record(1:lrecl), ier )
		    ELSE IF ( record(1:1) .eq. CHTAB .and.
     +				nb .eq. 0 ) THEN
			CALL WTPARA ( lunout, 'Body', ier )
			prvlst = 'Body'
			CALL PUTTAB ( record, lrecl, ier )
